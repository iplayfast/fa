		...
		where TInput:IEquatable<TInput>
	{
		...
		public bool IsDuplicate(FA<TInput,TAccept> rhs)
		{
			return null != rhs && IsAccepting==rhs.IsAccepting &&
				Equals(AcceptSymbol,rhs.AcceptSymbol) &&
		...
		public KeyValuePair<int, KeyValuePair<TInput[], int>[]>[] ToDfaTable(IDictionary<TAccept, int> symbolLookup = null)
		{
			var dfa = ToDfa();
				...
				using (var ye = rhs.GetEnumerator())
					while (xe.MoveNext() && ye.MoveNext())
					{
						if (xe.Current.Key != ye.Current.Key)
							return false;
						if (!_EqualsInput(xe.Current.Value, ye.Current.Value))
							return false;
					}
				return true;
			}
